...
auto Probability_evaluator::monte_carlo_evaluation(
...
) noexcept -> std::optional<std::pair<std::array<double, MAX_PLAYERS + 1>, std::size_t>>
{
...
Probability_evaluator::Probability_evaluator() noexcept = default;

void Probability_evaluator::set_sequnced_execution_policy(bool sequnced_policy) noexcept {
	is_sequnced_policy = sequnced_policy;
}

std::optional<std::pair<double, double>> Probability_evaluator::get_relative_probability(
	std::span<const Card> initial_player_cards,
	std::span<const Card> initial_table_cards,
	std::uint8_t number_of_players
) noexcept {
...
auto Probability_evaluator::get_absolute_probability(
	std::span<const Card> initial_player_cards,
	std::span<const Card> initial_table_cards,
	std::span<const std::pair<Card, Card>> initial_opponent_cards,
	std::uint8_t number_of_players
) noexcept -> std::optional<std::pair<std::array<double, MAX_PLAYERS>, std::size_t>>
{
...
void Probability_evaluator::generate_and_show_preflop_table(std::uint8_t number_of_players) noexcept {
